...
class Item:
    ...
    def get_position(self):
        # print("position get from the Item Class: ", self.position)
...
class Agent(Item):
    ...
    def move_request(self, direction_agent):
        ...
        if direction_agent > Agent.NB_ACTIONS/2 - 1:  # move the garabage
            moved = self.map[origin[0] - 1, origin[1]].get_item()
            origin[0] -= 1

        destination = origin[:] # move the agent 
        ...
        if move.check_validity(self.map):
            # print("the location after an action: ", destination)
            return move
        else:
    ...
    def act_clever(self, moral_value, norm_value, norm_civility): # Values.TO_TRASHCAN, Norms.NO_THROWING, Norms.NO_UNCIVILITY
        """
        ...
        # print("The agent chose the UP action outside of the IF-ELSE....................")

        if not self.map[pos[0] - 1, pos[1]].is_free(): # the next cell is not free: has garabage 

            if moral_value == 0 and norm_civility == 0: # the agent not throwing anything
                # print("===============getting into norm_civility mode ===============")
                if self.map[pos[0] - 1, pos[1] + 1].is_for_garbage(): # the next cell's right has bin
                    possible_actions.append(Agent.RIGHT + Agent.NB_ACTIONS/2) # push garabage right
                    print("get action from act_clever():", possible_actions)
                if self.map[pos[0] - 1, pos[1] - 1].is_for_garbage():  # the next cell's left has bin
                    possible_actions.append(Agent.LEFT + Agent.NB_ACTIONS / 2) # push garabage to the left
                    print("get action from act_clever():", possible_actions)
                if self.map[pos[0] - 2, pos[1]].is_for_garbage(): # the next next cell has bin
                    possible_actions.append(Agent.UP + Agent.NB_ACTIONS / 2) # push the garabage forward
                    print("get action from act_clever():", possible_actions)

                if norm_value < 0: # the agent throw the garage
                ...
                # print("=============== getting into the else() ====================")
                if self.map[pos[0] - 2, pos[1]].is_accessible(): # the next next cell is accessible
                    # print("+++++++have chose the if action+++++++", direction_chosen)
                    direction_chosen = Agent.UP + Agent.NB_ACTIONS/2 # push garabage forward
                    print("get action from act_clever():", direction_chosen)
                else: # if the next next cell is not accessible
                    if self.map[pos[0] -1, pos[1] + 1].is_for_garbage() and not self.map[pos[0] - 1, pos[1] + 1].is_accessible(): # the next right hand side cell is for garbage and not accessiable
                        direction_chosen = Agent.RIGHT + Agent.NB_ACTIONS / 2 # push the garbage to the right
                        print("get action from act_clever():", direction_chosen)
                        # print("---------------------have chose the second if action --------------------", direction_chosen)
                    else:
                        direction_chosen = Agent.LEFT + Agent.NB_ACTIONS / 2 # push the garbage to the left
                        print("get action from act_clever():", direction_chosen)
                        # print("=============have chose the else action================", direction_chosen)
        # print("The direction chosen by smart way: ", direction_chosen)
        return self.move_request(direction_chosen)
