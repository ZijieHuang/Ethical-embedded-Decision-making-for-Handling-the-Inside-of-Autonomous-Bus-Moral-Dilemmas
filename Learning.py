...
def deterministic_optimal_policy_calculator(Q, env, weights):
    ...
    print("policy from deterministic_optimal_policy_calculator(): ", policy)
    return policy
...
def choose_action(st, eps, q_table, weights, infoQ):
    ...
    eps = max(0.1, eps**infoQ[st[0],st[1],st[2]])  #  epsilon-greedy action selection procedure 
    NB_ACTIONS = 6
...
def q_learning(env, weights, alpha=0.8, gamma=0.7):
    ...
    # np.zeros([L_agent, R_agent, Garbage])
    ...
    for episode in range(1, max_episodes + 1):
        ...
        state = env.get_state() # get items and agents positions
        ...
        while not done and step_count < max_steps:
            ...
            actions.append(choose_action(state, epsilon, Q, weights, infoQ)) # random actions
            infoQ[state[0],state[1],state[2]] += 1.0


            # if is initial state, append another action
            if state[0] == 10 and state[1] == 11 and state[2] == 8:
                ...
                if np.random.randint(2) < 1:
                    ...
                    if env.is_deterministic:
                        ...
                        n = 3 # push garabage right
                    actions.append(n)

            print("actions from q_learning(): ",actions)
