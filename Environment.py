...
class Environment:
    ...
    def generate_agents(self, where_left=[4, 1], where_right=[4,2]):
        ...
        # print("agents are generated: ", agents)
        return agents
    ...
    def approve_move(self, move):
        ...
        if not ori.is_free(): # the original location is not free

            if dest.is_for_garbage(): # the next location is for garbage

                moved = move.get_moved()

                if isinstance(moved, Agent): # if the moved item is agent
                    ...
                    if dest.is_accessible(): 
                        ...
                        if not moved.get_map()[destiny[0], destiny[1]].is_free():
                            return False, is_damaged
                                ...
                                return False, is_damaged
                        else: # if the destination is free
                            if not dest.is_free():
                                moved.get_damaged()
    ...
    def act(self, actions):
        ...
        for i in range(len(self.agents)):  # agent L and agent R: 2---- 
          
            while len(actions) < len(self.agents): # if action didn't get appended 2 actions in q_learning(), which means is not initial state.
                actions.append(8000)
                # print("actions get append=============:",actions)
            # everytime will have two actions inputted, execute the first action as ramdon one, then the second action as clever one. 
            if actions[i] >= 0: 
                if actions[i] == 8000: # execute the appended action (the second one)
                    # print("get into acting clever --------")
    ...
    def to_reward(self, agent, action):
        ...
        reward = [0.0, 0.0] # first reward for the first objective. second reward for the second objective (moral related).
    ...
    def get_state(self):
        ...
        # print("the agent position got from get_state(): ",stator)
        return stator
    ...
    def step(self, actions):
        ...
        """
        self.act(actions)
        print("the actions got passed into step():", actions)
        rewards = list()
        dones = list()
        ...
        for agent, action in zip(self.agents, actions):
            ...
            dones.append(done)
        # print("the state after move a step: ", self.get_state())
